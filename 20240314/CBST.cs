namespace _20240314
{
    internal class CBST
    {
        ...
        작은 경우 왼쪽 자식노드, 큰 경우라면 오른쪽 자식노드로 이동
        1. 23과 17을 비교 ==> (23 > 17) -> 왼쪽으로감
        - 이제 오른쪽 서브트리는 탐색할 필요가 없음
        2. 11과 17을 비교 ==> (11 < 17) -> 오른쪽으로 감
        3. 19과 17을 비교 ==> (19 > 17) -> 왼쪽으로 감
        4. 17과 17을 비교 ==> 같음(찾음)
        ...
        작은 경우는 왼쪽 자식노드, 큰 경우 오른쪽 자식노드로 이동
        1. 23과 35를 비교 ==> (23 < 35) -> 오른쪽 이동
        2. 38과 35를 비교 ==> (38 > 35) -> 왼쪽 이동
        3. 31이랑 35를 비교 ==>(31 < 35) -> 오른쪽 이동
        4. 자리가 비었으면 삽입
        ...
        2. 자식이 1개인 노드의 삭제
        -삭제하는 노드의 부모와 자식을 연결 후 삭제
        - 38 삭제 -> 23과 38의 자식(31) 연결
        ...
        2. 자식이 2개인 노드 삭제
        - 삭제하는 노드를 기준으로 오른쪽 자신중 가장 작은 값 노드와 교체 후 삭제
                    23                                     
              ┌──────┴──────┐                                                                                   
              11            38                                                          
           ┌──┴──┐       ┌──┴──┐                                                                          
        ...
        public class BinarySearchTree<T> where T : IComparable<T>
        {
            ...
            private void EraseNode(Node node)
            {
                ...
                // 자식이 2개인 경우(node.left != null && node.right != null)
                else
                {
